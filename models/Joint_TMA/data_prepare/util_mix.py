...
def get_one_mix_npy_info(json_graph_dict, n_node, n_edge_types, max_word_num):
    ...
    for i in range(0, node_num):
        ...
        if 'snode_cfg' in json_graph_dict[str(i)].keys():
            snode_list = json_graph_dict[str(i)]['snode_cfg']
            for j in range(0, len(snode_list)):
                if snode_list[j] < n_node:
                    snode = snode_list[j]
                    save_edge_digit_list.append([i, snode, 0])

        if 'snode_pdgc' in json_graph_dict[str(i)].keys():
            snode_list = json_graph_dict[str(i)]['snode_pdgc']
            for j in range(0, len(snode_list)):
                if snode_list[j] < n_node:
                    snode = snode_list[j]
                    save_edge_digit_list.append([i, snode, 1])

        if 'snode_pdgd' in json_graph_dict[str(i)].keys():
            snode_list = json_graph_dict[str(i)]['snode_pdgd']
            for j in range(0, len(snode_list)):
                if snode_list[j] < n_node:
                    snode = snode_list[j]
                    save_edge_digit_list.append([i, snode, 1])
...
def create_dict_file(args):
    ...
    # vocab_ir_file_path = '../dataset/data/' + args.vocab_ir_file
...
def txt2json(args, ir_txt_file_path):
    ...
    with open(ir_txt_file_path, 'r') as ir_txt_file:
        ...
        for i in range(0, len(ir_lines)):
            if ir_lines[i][0:9] == 'BeginFunc' and i != 0:
                end_index = i
                mark_list.append([start_index, end_index])
                start_index = i
                ir_cnt += 1
