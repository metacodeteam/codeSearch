...
def test(config, model, device):
    ...
    test_set = eval(config['dataset_name'])(config, data_path,
                                            config['test_token'], config['token_len'],
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='TokenEmbeder', help='model name')
    parser.add_argument('-d', '--dataset', type=str, default='dataset', help='name of dataset.java, python')
    parser.add_argument('--reload_from', type=int, default=140, help='epoch to reload from')
    parser.add_argument('-g', '--gpu_id', type=int, default=0, help='GPU ID')
    parser.add_argument('-v', "--visual", action="store_true", default=False,
                        help="Visualize training status in tensorboard")

    parser.add_argument('--models_path', type=str, default='models_no_transformerout')
...
if __name__ == '__main__':
    ...
    ckpt = f'./output/MIXEmbeder/pre_dataset/{args.models_path}/epo{args.reload_from}.h5'
