...
class JointEmbeder(nn.Module):
    def __init__(self, config):
        ...
        self.name_encoder = SeqEncoder(config['n_name_words'], config['emb_size'], config['lstm_dims'])
        self.desc_encoder = SeqEncoder2(config['n_desc_words'], config['emb_size'], config['n_hidden'])
    ...
    def desc_encoding(self, desc, desc_len):
        ...
        if self.conf['use_desc_attn']:
            ...
            unpack_len_list = desc_len.long().to(self.device)
            range_tensor = torch.arange(seq_len).to(self.device)
            mask_1forgt0 = range_tensor[None, :] < unpack_len_list[:, None]
            mask_1forgt0 = mask_1forgt0.reshape(-1, seq_len)

            desc_sa_tanh = torch.tanh(
                self.self_attn2(desc_feat.reshape(-1, self.n_hidden)))  # [(batch_sz * seq_len) x n_hidden]
    ...
    def forward(self, name, name_len, desc_anchor, desc_anchor_len, desc_neg,
                desc_neg_len):
        # code_repr: [batch_sz x n_hidden]  
        code_repr = self.code_encoding(name, name_len)
