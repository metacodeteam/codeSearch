...
def train(args):
    ...
    for epoch in range(int(args.reload_from) + 1, config['nb_epoch'] + 1):
        ...
        if epoch % args.val_every == 0:
            ...
            data_loader_val = torch.utils.data.DataLoader(dataset=val_set, batch_size=32, shuffle=False,
            ...
            for batch_val in data_loader_val:
                ...
                code_batch = [tensor.to(device) for tensor in batch_val[:2]]
                # batch[7:9]: good_desc, good_desc_len
                desc_batch = [tensor.to(device) for tensor in batch_val[2:4]]
...
def parse_args():
    ...
    parser.add_argument('-v', "--visual", action="store_true", default=True,
