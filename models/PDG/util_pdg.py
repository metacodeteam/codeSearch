...
def get_one_pdg_npy_info(json_pdg_dict, n_node, n_edge_types, state_dim, max_word_num):
    node_num = min(len(json_pdg_dict), n_node)
    ...
    while node_index < node_num:
        if str(i) not in json_pdg_dict.keys():
            i += 1
            continue

        word_list = json_pdg_dict[str(i)]['wordid']
        ...
        if 'snode_pdgc' in json_pdg_dict[str(i)].keys():
            snode_list = json_pdg_dict[str(i)]['snode_pdgc']
            for k in range(0, min(n_node, len(snode_list))):
                snode = snode_list[k]
                if snode < n_node:
                    save_edge_digit_list.append([i, snode, 0])

        if 'snode_pdgd' in json_pdg_dict[str(i)].keys():
            snode_list = json_pdg_dict[str(i)]['snode_pdgd']
            for k in range(0, min(n_node, len(snode_list))):
                snode = snode_list[k]
                if snode < n_node:
