...
import configs
# from util_pdg import get_pdg_npy_info, get_one_pdg_npy_info
from util_pdg import get_one_pdg_npy_info
...
class CodeSearchDataset(data.Dataset):
    ...
    """
    def __init__(self, config, data_dir, f_pdgs, max_node_num, f_descs=None, max_desc_len=None):
        ...
        self.graph_dict = json.loads(open(data_dir + f_pdgs, 'r').readline())
    ...
    def __getitem__(self, offset):          
        ...
        adjmat, init_input, node_mask = get_one_pdg_npy_info(self.graph_dict[str(offset)],
                    self.max_node_num, self.n_edge_types, self.state_dim, self.max_word_num)
...
if __name__ == '__main__':
    device = 'cpu'
    config = getattr(configs, 'config_PDGEmbeder')()
    input_dir = './data/dataset/'

    train_set = CodeSearchDataset(config, input_dir, 'train_pdg.json', 512, 'train.desc.h5', 30)
    train_data_loader = torch.utils.data.DataLoader(dataset=train_set, batch_size=5, shuffle=False, drop_last=False, num_workers=0)
