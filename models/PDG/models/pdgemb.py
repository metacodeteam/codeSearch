...
class PDGEmbeder(nn.Module):
    def __init__(self, config):
        ...
        self.pdg_attn_mode = config['pdg_attn_mode']

        self.pdg_encoder = GGNN(self.conf)
    ...
    def code_encoding(self, pdg_init_input_batch, pdg_adjmat_batch, pdg_node_mask):
        
        batch_size = pdg_node_mask.size()[0]

        # code_feat: [batch_size x n_node x state_dim]
        code_feat = self.pdg_encoder(pdg_init_input_batch, pdg_adjmat_batch, pdg_node_mask) # forward(prop_state, A, node_mask)
        ...
        mask_1forgt0 = pdg_node_mask.bool().reshape(-1, node_num)
        ...
        for _i in range(batch_size):
            ...
            if self.pdg_attn_mode == 'sigmoid_scalar':
    ...
    def forward(self, pdg_init_input_batch, pdg_adjmat_batch, pdg_node_mask, desc_anchor, desc_anchor_len, desc_neg, desc_neg_len):
        # code_repr: [batch_sz x n_hidden]  
        pdg_repr = self.code_encoding(pdg_init_input_batch, pdg_adjmat_batch, pdg_node_mask)
        ...
        # sim: [batch_sz]
        anchor_sim = self.similarity(pdg_repr, desc_anchor_repr)
        neg_sim = self.similarity(pdg_repr, desc_neg_repr)
