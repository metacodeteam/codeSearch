...
class MultiEmbeder(nn.Module):
    ...
    def code_encoding(self, tree, tree_node_num, graph_init_input_batch, graph_adjmat_batch,
                      graph_node_mask):
        ...
        ast_enc_hidden = self.ast_encoder.init_hidden(
            tree.graph.number_of_nodes())  # use all_node_num to initialize h_0, c_0
        ...
        if self.conf['use_ast_attn']:
            ...
            for _i in range(batch_size):
                ...
                ast_attn_this_sample_h = torch.bmm(ast_attn_weight.reshape(1, 1, node_num),
                                                   this_sample_h.reshape(1, node_num, self.n_hidden)).reshape(1,
                                                                                                              self.n_hidden)
        ...
        # concat_feat: [batch_size x (n_hidden * 2)]
        concat_feat = torch.cat((ast_feat_attn, graph_feat_attn), 1)
    ...
    def forward(self, tree, tree_node_num, cfg_init_input, cfg_adjmat, cfg_node_mask, desc_anchor,
                desc_anchor_len, desc_neg, desc_neg_len):
        # code_repr: [batch_size x n_hidden]
        code_repr = self.code_encoding(tree, tree_node_num, cfg_init_input, cfg_adjmat, cfg_node_mask)
