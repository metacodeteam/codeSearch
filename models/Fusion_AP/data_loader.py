...
import configs
from data_prepare.util_graph import get_one_graph_npy_info
...
def batcher(device):
    def batcher_dev(batch):
        ...
        tree, tree_node_num, init_input, adjmat, node_mask, good_desc, good_desc_len, bad_desc, bad_desc_len = zip(
            *batch)
        ...
        return tree, tree_node_num, init_input, adjmat, node_mask, good_desc, good_desc_len, bad_desc, bad_desc_len
...
class CodeSearchDataset(data.Dataset):
    ...
    def __init__(self, config, data_dir, f_ast, f_ast_dict, f_cfgs, max_node_num, f_desc,
                 max_desc_len):
    ...
    def __getitem__(self, offset):
        ...
        init_input, adjmat, node_mask = get_one_graph_npy_info(self.graph_dict[str(offset)],
...
if __name__ == '__main__':
    ...
    train_set = CodeSearchDataset(config, input_dir,
                                  'train.token.h5', 50, 'train.ast.json', 'vocab.ast.json', 'train.cfg.json', 512,
                                  'train.desc.h5', 30)
    ...
    for batch in train_data_loader:
        # print(i)
        print(batch)
        tokens, tok_len, tree, tree_node_num, init_input, adjmat, node_mask, good_desc, good_desc_len, bad_desc, bad_desc_len = [
            t for t in batch]
        print(batch)
