...
import configs
from data_prepare.util_mix import get_one_mix_npy_info
...
def batcher(device):
    def batcher_dev(batch):
        ...
        tree, tree_node_num, good_desc, good_desc_len, bad_desc, bad_desc_len = zip(*batch)
...
class CodeSearchDataset(data.Dataset):
    ...
    """
    def __init__(self, config, data_dir, f_ast, f_ast_dict, f_desc, max_desc_len):
...
def tuplelist2tensor_float(float_tuple_list):
    float_numpy = np.zeros([len(float_tuple_list), len(float_tuple_list[0])])
    for index, value in enumerate(float_tuple_list):
...
def tuple3list2tensor_long(long_tuple_3list):
    float_numpy = np.zeros([len(long_tuple_3list), long_tuple_3list[0].shape[0], long_tuple_3list[0].shape[1]])
    for index, value in enumerate(long_tuple_3list):
        float_numpy[index] = value
    long_tensor = torch.from_numpy(float_numpy).type(torch.LongTensor)
    return long_tensor
...
if __name__ == '__main__':
    device = 'cpu'
    config = getattr(configs, 'config_Joint_TMA')()
    input_dir = './data/dataset/'
    
    train_set = CodeSearchDataset(config, input_dir, 
                        'train.token.h5', 50, 'train.ast.json', 'vocab.ast.node.json', 'train.mix.json', 160, 'train.desc.h5', 30)
    ...
    for batch in train_data_loader:
        #print(i)
        print(batch)
        tokens, tok_len, tree, tree_node_num, anno, adjmat, node_mask, good_desc, good_desc_len, bad_desc, bad_desc_len = [t for t in batch]
        print(batch)
