    ...
    for epoch in range(int(args.reload_from) + 1, config['nb_epoch'] + 1):
        ...
        if epoch % args.val_every == 0:
            ...
            data_loader_val = torch.utils.data.DataLoader(dataset=val_set, batch_size=32,
                                                      collate_fn=batcher(device), shuffle=False, drop_last=True,
                                                      num_workers=0)
            ...
            for batch_val in data_loader_val:
                code_batch = [tensor for tensor in batch_val[:2]]
                desc_batch = [tensor for tensor in batch_val[2:4]]
                val_loss = model.forward(*[tensor for tensor in batch_val])
                ...
                n_processed += batch_val[2].size(0)  # +batch_size
                # if n_processed >= 2000:
                #     break
