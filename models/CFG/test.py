...
def test(config, model, device):
    ...
    for batch in data_loader:
        ...
        code_batch = [tensor.long().to(device) for tensor in batch[:3]]
        # batch[3:5]: good_desc, good_desc_len
        desc_batch = [tensor.long().to(device) for tensor in batch[3:5]]
        with torch.no_grad():
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', type=str, default='test', help='name of dataset.java, python')
...
if __name__ == '__main__':
    ...
    ckpt=f'./output/{args.model}/train/models/epo{args.reload_from}.h5'
