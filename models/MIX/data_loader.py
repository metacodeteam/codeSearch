...
import configs
from util_mix import get_one_mix_npy_info
...
class CodeSearchDataset(data.Dataset):
    ...
    def __init__(self, config, data_dir, f_mixs, max_node_num, f_descs=None, max_desc_len=None):
        ...
        self.graph_dict = json.loads(open(data_dir + f_mixs, 'r').readline())
    ...
    def __getitem__(self, offset):
        ...
        anno, adjmat, node_mask = get_one_mix_npy_info(self.graph_dict[str(offset)], self.max_node_num,
                                                       self.n_edge_types, self.max_word_num)
...
if __name__ == '__main__':
    device = 'cpu'
    config = getattr(configs, 'config_MIXEmbeder')()
    input_dir = './data/github1/'

    test_set = CodeSearchDataset(config, input_dir, 'test.ir.json', 160, 'test.desc.h5', 30)
    test_data_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=1, shuffle=False, drop_last=False,
                                                   num_workers=1)
    ...
    for batch in test_data_loader:
        # print(batch)
        anno, adjmat, node_mask, good_desc, good_desc_len, bad_desc, bad_desc_len = [tensor.to(device) for tensor in
                                                                                     batch]
        # print(anno)
        print(adjmat)
