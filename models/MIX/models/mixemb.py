...
class MIXEmbeder(nn.Module):
    def __init__(self, config):
        super(MIXEmbeder, self).__init__()
        ...
        self.mix_attn_mode = config['mix_attn_mode']


        self.mix_encoder = GGNN(self.conf)
    ...
    def code_encoding(self, mix_init_input_batch, mix_adjmat_batch, mix_node_mask):
        batch_size = mix_node_mask.size()[0]

        # code_feat: [batch_size x n_node x state_dim]
        # code_feat = self.mix_encoder(mix_init_input_batch, mix_adjmat_batch)  # forward(annotation, A)
        ...
        mask_1forgt0 = mix_node_mask.bool().reshape(-1, node_num)
        ...
        #         self.linear_single_modal(F.dropout(code_feat, self.dropout, training=self.training)))
            # code_feat = self.linear_single_modal(F.dropout(code_feat, self.dropout, training=self.training))

        code_sa_tanh = torch.tanh(
            self.self_attn(code_feat.reshape(-1, self.n_hidden)))  # [(batch_size * n_node) x n_hidden]
        ...
        for _i in range(batch_size):
            ...
            if self.mix_attn_mode == 'sigmoid_scalar':
        ...
        if self.conf['transform_every_modal']:
            ...
            # )
            self_attn_code_feat = self.linear_attn_out(
                F.dropout(self_attn_code_feat, self.dropout, training=self.training))
    ...
    def forward(self, mix_anno, mix_adjmat, mix_node_mask, desc_anchor, desc_anchor_len, desc_neg, desc_neg_len):
        # code_repr: [batch_size x n_hidden]
        mix_repr = self.code_encoding(mix_anno, mix_adjmat, mix_node_mask)
        ...
        anchor_sim = F.cosine_similarity(mix_repr, desc_anchor_repr)
        neg_sim = F.cosine_similarity(mix_repr, desc_neg_repr)
